...
def process_llm_response(llm_response, width, out_file):
    ...
    with open(out_file, 'a', encoding="utf-8") as output:
        ...
        output.write(
            wrap_text_preserve_newlines(llm_response["question"], width))
        output.write("\nResponse:\n")
        output.write(wrap_text_preserve_newlines(llm_response["answer"],
                                                 width))
        output.write("\n\nSources:\n\n")
        for source in llm_response["source_documents"]:
            if "page" in source.metadata:
                output.write(f"Page number:{source.metadata['page']} Document:"
...
def load_llm(config):
    ...
    if "llama" in model_id.lower():
        ...
        # raise ValueError(
        #     "Falcon is currently unavailable, please use a Llama-2 model.")
        ...
        raise ValueError(
            f"Model in '{model_id}' not supported, must be Llama-2"
            "or Falcon")
...
def read_db(db, config):
    ...
    vectordb = Chroma(embedding_function=instructor_embeddings,
                      persist_directory=db)
    retriever = vectordb.as_retriever(search_type=config["search_type"],
...
def create_memory(model, tokenizer, config):
    ...
    Args:
        ...
        config: Configuration data with parameter values.
    """
    summary_pipe = pipeline("text-generation",
                            model=model,
                            tokenizer=tokenizer,
    ...
    summary_prompt = PromptTemplate(template=utils.SUMMARY_PROMPT_TEMPLATE,
                                    input_variables=["summary", "new_lines"])

    return ConversationSummaryMemory(llm=summary_llm,
...
def qa_generator(model, tokenizer, config, retriever, ipynb=False):
    ...
    qa_prompt = PromptTemplate(template=utils.QA_PROMPT_TEMPLATE,
                               input_variables=["context", "question"])

    # Generating a standalone question
    question_generator_pipe = pipeline("text-generation",
                                       model=model,
                                       tokenizer=tokenizer,
                                       device_map=config["q_device_map"],
                                       pad_token_id=tokenizer.eos_token_id,
                                       max_length=config["q_max_length"])
    question_generator_llm = HuggingFacePipeline(
        pipeline=question_generator_pipe)
    question_generator = LLMChain(llm=question_generator_llm,
    ...
    doc_chain_pipe = pipeline("text-generation",
                              model=model,
                              tokenizer=tokenizer,
    ...
    doc_chain = load_qa_chain(doc_chain_llm,
                              chain_type="stuff",
                              prompt=qa_prompt)
...
if __name__ == "__main__":
    if len(sys.argv) < 4 or len(sys.argv) > 6:
        ...
        print(
            "Usage: python query.py <db> <query_file> <output> [config file]")
        sys.exit(1)
